...
async function mailgunForecast(current, forecast, email) {
  ...
  const msg = {
    ...
    // text: `Here is your custom weather forecast from NotifyMe-US:

    // The weather in ${current.town} is currently ${current.weather}. The high temperature for today will be ${current.fahrenheitRoundedHigh} degrees Fahrenheit, and the low temperature will be ${current.fahrenheitRoundedLow} degrees Fahrenheit. The humidity will be ${current.humidity}%, and the wind speed will be ${current.windMphFormatted} mph. There will be ${current.cloudCoverage}% cloud coverage.
  ...
  mailgun.messages().send(msg, (error, body) => {
    if (error) {
      console.error(error);
      throw 'An error has occured with your request';
    } else {
